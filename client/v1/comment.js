...
Comment.create = function(session, mediaId, text) {
  return new Request(session)
    ...
    .setData({
      ...
      src: 'profile',
      comment_text: text,
      idempotence_token: crypto.createHash('md5').update(text)
        .digest('hex')
    })
    .signPayload()
    .send()
...
Comment.delete = function(session, mediaId, commentId) {
  return new Request(session)
    ...
    .setData({
      media_id: mediaId,
      src: 'profile',
      idempotence_token: crypto.createHash('md5').update(commentId)
        .digest('hex')
    })
    .signPayload()
    .send()
...
Comment.bulkDelete = function(session, mediaId, commentIds) {
  return new Request(session)
    ...
    .setData({
      ...
      src: 'profile',
      idempotence_token: crypto.createHash('md5').update(commentIds.join(','))
        .digest('hex')
    })
    .signPayload()
    .send()
